...
function getResponses(finalElement) {
    ...
    for (let iterator1 = 0; iterator1 < finalElement; iterator1++) {
    ...
    if (responses.length == finalElement){
        validation = true
    }
...
function sendReview(validation,response,idQuestions){
    let form = document.createElement('form')
    form.style.display='none'
    form.method= 'POST'
    form.action= '/controller/router.php'
    ...
    validationInput.name = 'validation'
    validationInput.value = validation
...
function nextQuestion(idElement,finalElement){
    ...
    if (idElement == finalElement){
        ...
        sendReview(validation,response,idQuestions)
    } else {   
